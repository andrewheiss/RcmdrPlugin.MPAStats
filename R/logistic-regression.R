...
logitWords <- function(x){
    ...
    for(i in seq(2,nrow(x$coefficients))[-factor.coef]){
        ...
        if(pval >=  alpha){
            ...
            cat("Test Results: Controlling for all other variables in the model, ",xname," has a statistically significant ",sign," relationship with ",yname,". For a one unit increase in ",xname,", the likelihood of ",yname," ",updown," by a factor of ",round(exp(beta),3),". (z=",round(x$coefficients[i,3],3),", p=",round(x$coefficients[i,4],3),"). \n \n",sep="")
        }
    }
}
